...
class FCMAlgorithm(Algorithm):
    ...
    Follows the MATLAB reference:
        [Rfcm, Dfcm, Cfcm] = FCM2(ROI, C, IterQuintanilla);

    The output is a binary MC mask where pixels in the brightest cluster
    are marked as microcalcification candidates:
        MATLAB: logical(Rfcm == max(max(Rfcm)))
    ...
    def _update_centroids(Z: torch.Tensor, U: torch.Tensor, m: float) -> torch.Tensor:
        ...
        Um = U ** m  # (c, N)
        denom = Um.sum(dim=1, keepdim=True) + 1e-10  # (c, 1)
        return (Um @ Z) / denom  # (c, d)
    ...
    def apply(self, image: Image, output: Image):
        ...
        V = self._update_centroids(Z, U, self.m)
        ...
        for iteration in range(self.max_iter):
            ...
            U_new = self._update_membership(D2, self.m)
            V = self._update_centroids(Z, U_new, self.m)
